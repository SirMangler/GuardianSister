    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
		...
		if (event.getAuthor().getId().equalsIgnoreCase("417888570603339780")) { 
	        ...
	        if (msg.replace(" ", "").matches(".*(j|🇯|\\|)+(o|🇴|0)+(n|🇳|π)+(i|!|🇮|\\||1)+(l|🇱|!|\\||l)+.*") 
	        		...
	        		|| raw_chars.contains("☺️⚐☠️☠️✋☹️") 
	        		|| raw_chars.contains("🙰□︎■︎■︎♓︎●︎") 
	        		|| raw_chars.contains("🇯 🇴 🇳 🇳 🇮 🇱") 
	        		|| msg.contains(".--- --- -. -. .. .-..") 
	        		|| msg.contains("_|0|\\\\||\\\\|!|_") 
	        		|| msg.contains("_)0|\\\\||\\\\|!1")
	        	...
	        	event.getGuild().getTextChannelById("466824732856025099")
	        		.sendMessage("Matched: `"+ event.getMessage().getContentRaw() + "` (Count: "+count+")").queue();
	        }
		}
    }
}
